...
class XMLGenerator:
    ...
    def background_widget():
        ...
        frame_widget = etree.SubElement(background_item, "widget", attrib={
            "class": "QFrame",
            "name": "Background",
        })
...
class Converters:
    ...
    def visibility_converter(self, visibility_string, pydm):
        ...
        for edmline in visibility_list:
            ...
            if has_vis_min and has_vis_max:
                ...
                pydm.writelines("&quot;expression&quot;: &quot;ch[0]&quot;, ")
    ...
    def widgets_converter(self, line_count):
        ...
        for edmline in self.edmfile[line_count:]:
            ...
            if is_in_widget and edmline.startswith("endObjectProperties"):
                ...
                elif widget_string.__contains__("object activeButtonClass") and not widget_string.__contains__("push"):
                    self.Enum_Combo_Box_count = self.Enum_Combo_Box_count + 1
                    self.enum_combo_box_converter(widget_string)
                ...
                elif widget_string.__contains__("object activeMotifSliderClass"):
                    self.Slider_count = self.Slider_count + 1
                    self.slider_converter(widget_string)
    ...
    def rectangle_converter(self, widget_string):
        ...
        pydm.writelines([f'<widget class="PyDMDrawingRectangle" name="PyDMDrawingRectangle_{self.Rectangle_count}">\n'])
            ...
            # convert the text on the button of the shell command
                ...
                if widget_string.__contains__("pydm"):
                    with open(csv_file, "a") as csvfile:
                        csvfile.writelines(
                            edmline
                            + ":  Contains a command to launch a pydm screen, advised to change this to a PyDMRelatedDisplayButton\n"
                        )
                    csvfile.close()
    ...
    def enum_combo_box_converter(self, widget_string):
        ...
        widget_list = self.str_to_list(widget_string)
        pydm = open(pydm_file, "a")
        pydm.writelines([f'<widget class="PyDMEnumComboBox" name="PyDMEnumComboBox_{self.Enum_Combo_Box_count}">\n'])
        for edmline in widget_list:
            if edmline.startswith(("x ", "y ", "w ", "h ")):
                self.geometry_converter(edmline, pydm)
            elif edmline.startswith("controlPv "):
    ...
    def edm_related_converter(self, widget_string):
        ...
        if not macros_are_too_long:
            ...
            self.Shell_Command_count = self.Shell_Command_count + 1
            pydm = open(pydm_file, "a")
            pydm.writelines([f'<widget class="PyDMShellCommand" name="PyDMShellCommand_{self.Shell_Command_count}">\n'])
            for edmline in widget_list:
                if edmline.startswith(("x ", "y ", "w ", "h ")):
                    self.geometry_converter(edmline, pydm)
    ...
    def poly_line_converter(self, widget_string, x_points, y_points):
        ...
        Future future self here, also confirming that that was rude of past self :(
        """
        widget_list = self.str_to_list(widget_string)
        pydm = open(pydm_file, "a")
        pydm.writelines([f'<widget class="PyDMDrawingPolyline" name="PyDMDrawingPolyline_{self.Poly_Line_count}">\n'])

        for edmline in widget_list:
            if edmline.startswith(("x ", "y ", "w ", "h ")):
                self.geometry_converter(edmline, pydm, 10, 10)
                if edmline.startswith("x"):
    ...
    def enum_button_converter(self, widget_string):
        ...
        # if it is a radio button set the style to radio button
        if widget_string.__contains__("activeRadioButtonClass"):
            pydm.writelines(
                [
        ...
        for edmline in widget_list:
            ...
            # check horizontal or vertical
                ...
                pydm.writelines('<property name="orientation" stdset="0">\n<enum>Qt::Horizontal</enum>\n</property>\n')
    ...
    def slider_converter(self, widget_string):
        ...
        widget_list = self.str_to_list(widget_string)
        pydm = open(pydm_file, "a")
        pydm.writelines([f'<widget class="PyDMSlider" name="PyDMSlider_{self.Slider_count}">\n'])
...
def locate(patterns, root=os.curdir, recursive=True):
    ...
    if not os.path.exists(folder) or not os.path.isdir(folder):
        raise ValueError(
            "{} ({})".format(
                "Not a folder: -r option requires a folder, run without -r for individual files",
                folder,
            )
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=5,
        help="EDM font is larger than PyDM font, amount to decrease the font by, defaults to 5. Larger numbers decrease font more",
    )
    parser.add_argument(
        "-e",
        "--edm-related",
    ...
    parser.add_argument(
        ...
        "--no-csv",
        action="store_true",
        help="No csv file with overflow widgets is created",
    )
    ...
    parser.add_argument(
        ...
        "--force",
        action="store_true",
        help="Overrides files of the same name without asking",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="autogen",
        help="Changes the tag added to the output PyDM file {file}_{tag}.ui.  Defaults to _autogen",
    )

    parser.add_argument(
        "--remove-subsys",
        action="store_true",
        help="removes the subsystem name from created files e.g. pps_{file}.edl -> {file}.ui",
    )
    parser.add_argument(
        "--hide_related",
        action="store_true",
    ...
    parser.add_argument(
        "input",
        help="Target file to convert, needs to be .edl. If -r option is used all .edl files in given directory tree will be converted",
    )
    parser.add_argument(
        "destination",
        type=str,
        default="default",
        help="Destination directory for output file(s).  Use '.' or 'current' to get output to the directory you are running the script in.  Works with absolute and relative paths",
    )
    ...
    if not args.recursive:
        if not args.input.endswith(".edl"):
            ...
            sys.exit(1)
    if args.show_related and args.hide_related:
    ...
    if args.destination == "." or args.destination == "current":
        ...
        save = input(
            f"WARNING: (-d, -destination) option expects a directory, not a file.  Save output file(s) to directory '{args.destination}'? (y/n)"
        )
        if save != "y":
            print("\nexiting program\n")
            sys.exit()
