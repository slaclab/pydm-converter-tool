...
class Converters(object):
    ...
    def widgets_converter(self, line_count):
        ...
        for edmline in self.edmfile[line_count:]:
            ...
            if is_in_widget and edmline.startswith('endObjectProperties'):
                ...
                elif widget_string.__contains__('object activeXTextDspClass') and not widget_string.__contains__('editable'):
                    self.PydmLabel_count = self.PydmLabel_count + 1
                    self.pydm_label_converter(widget_string)

                elif widget_string.__contains__('object activeXTextDspClass') and widget_string.__contains__('editable'):
                ...
                elif widget_string.__contains__('object activeMessageButtonClass') and not widget_string.__contains__('toggle'):
                    self.Push_Button_count = self.Push_Button_count + 1
                    self.button_converter(widget_string)

                elif widget_string.__contains__('object activeMessageButtonClass') and widget_string.__contains__('toggle'):
    ...
    def pydm_related_converter(self, widget_string):
        ...
        macros = []
        pydm = open(pydm_file, 'a')
        pydm.writelines([f"<widget class=\"PyDMRelatedDisplayButton\" name=\"PyDMRelatedDisplayButton_{self.Pydm_Related_count}\">\n"])
    ...
    def edm_related_converter(self, widget_string):
        ...
        if not macros_are_too_long:
            ...
            pydm.writelines([f"<widget class=\"PyDMEDMDisplayButton\" name=\"PyDMEDMDisplayButton_{self.Edm_Related_count}\">\n"])
            if (widget_string.__contains__('invisible') and not args.show_related) or (args.hide_related):
                pydm.writelines(["<property name=\"showIcon\" stdset=\"0\">\n"
                                "<bool>false</bool>\n"
                                "</property>)\n"])
    ...
    def real_line_converter(self, widget_string, x_points, y_points):
        ...
        if y_points[0] == y_points[1]:
            ...
            angle = math.degrees(math.atan((float(y_points[0])-float(y_points[1]))/(float(x_points[1])-float(x_points[0]))))
    ...
    def enum_button_converter(self, widget_string):
        ...
        if widget_string.__contains__("activeRadioButtonClass"):
            pydm.writelines(["<property name=\"widgetType\" stdset=\"0\">\n",
...
def locate(patterns, root=os.curdir, recursive=True):
    ...
    if not os.path.exists(folder) or not os.path.isdir(folder):
        raise ValueError('{} ({})'.format('Not a folder: -r option requires a folder, run without -r for individual files', folder))
